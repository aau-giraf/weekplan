// COMPONENT MADE BY REACT-NATIVE-GESTURE-HANDLER LIBRARY
...
import { GestureStateChangeEvent, GestureUpdateEvent } from "react-native-gesture-handler";
...
export interface SwipeableProps extends Pick<PanGestureHandlerProperties, SwipeableExcludes> {
  /**
...
const Swipeable = forwardRef<SwipeableMethods, SwipeableProps>(function Swipeable(
  ...
  const { friction = defaultProps.friction, overshootFriction = defaultProps.overshootFriction } = props;
  ...
  const updateAnimatedEvent = () => {
    ...
    showLeftProgress.value =
      leftWidth.value > 0 ? interpolate(appliedTranslation.value, [-1, 0, leftWidth.value], [0, 0, 1]) : 0;
    ...
    showRightProgress.value =
      rightWidth.value > 0 ? interpolate(appliedTranslation.value, [-rightWidth.value, 0, 1], [1, 0, 0]) : 0;
  ...
  const leftElement = renderLeftActions && (
    <Animated.View style={[styles.leftActions, leftAnimatedStyle]}>
      ...
      <View onLayout={({ nativeEvent }) => (leftWidth.value = nativeEvent.layout.x)} />
    </Animated.View>
  );
  ...
  const rightElement = renderRightActions && (
    <Animated.View style={[styles.rightActions, rightAnimatedStyle]}>
      ...
      <View onLayout={({ nativeEvent }) => (rightOffset.value = nativeEvent.layout.x)} />
    </Animated.View>
  );
  ...
  const handleRelease = (event: GestureStateChangeEvent<PanGestureHandlerEventPayload>) => {
    "worklet";
  ...
  const swipeableComponent = (
    <GestureDetector gesture={panGesture} touchAction="pan-y">
      <Animated.View {...remainingProps} onLayout={onRowLayout} style={[styles.container, containerStyle]}>
        {leftElement}
        {rightElement}
